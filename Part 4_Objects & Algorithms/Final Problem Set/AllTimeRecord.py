...
def all_time_record(opponent):
    df = pd.read_csv('../resource/lib/public/georgia_tech_football.csv')

    opponent_games = df[df["Opponent"] == opponent]

    wins = (opponent_games["Points For"] > opponent_games["Points Against"]).sum()
    losses = (opponent_games["Points For"] < opponent_games["Points Against"]).sum()
    ties = (opponent_games["Points For"] == opponent_games["Points Against"]).sum()
