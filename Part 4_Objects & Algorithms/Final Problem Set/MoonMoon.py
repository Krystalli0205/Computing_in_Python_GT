...
def generate_name(filename, name_string):
    file = open(filename, "r")
    lines = file.readlines()

    name_list = name_string.split()
    firstname_initial = name_list[0][0]
    lastname_initial = name_list[1][0]

    char_to_index = {}
    for i in range(65, 26+65):
        char_to_index[chr(i)] = i-65

    firstname_index = char_to_index[firstname_initial]
    lastname_index = char_to_index[lastname_initial] + 26

    firstname_hero = lines[firstname_index].strip()
    lastname_hero = lines[lastname_index].strip()

    return f"{firstname_hero} {lastname_hero}"
