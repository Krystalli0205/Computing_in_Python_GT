...
def check_plagiarism(file_1, file_2):
    file1 = open(file_1, "r")
    file2 = open(file_2, "r")
    words_list1 = file1.read().split()
    words_list2 = file2.read().split()

    longest_common_substring = ""

    for i in range(len(words_list1)):
        for j in range(i+5, len(words_list1)+1):
            substring = " ".join(words_list1[i:j])
            if substring in " ".join(words_list2) and len(substring) > len(longest_common_substring):
