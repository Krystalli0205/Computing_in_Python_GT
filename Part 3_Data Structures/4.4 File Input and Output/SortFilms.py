...
def sort_films(sour_filename, des_filename):
    input = open(sour_filename, "r")
    output = open(des_filename, "w")
    movie = []
    for line in input:
        name, gross = line.strip().split("\t")
        gross = int(gross.replace(",", "").replace("$", ""))
        movie.append((name, gross))
    
    movie.sort(key = lambda x: x[1], reverse=True)

    for name, gross in movie:
        output.write(f"{name}\t{gross}\n")
    output.close()
    input.close()
